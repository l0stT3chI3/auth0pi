//Importing modules
const {signupValidate, loginValidate, resetValidate} = require("../Models/validate");
const user = require("../Models/user");
const crypto = require("crypto");
const {SendVerificationEmail, SendPasswordEmail} = require("../Models/email");
...
exports.verify = catchAsync (async (req,res) => {
    ...
    const User = await user.findOne({uniqueString: token, date:{$gte: date}});
    if(User) {
        User.isvalid = true;
        await User.save();
...
exports.changePassword = catchAsync(async (req,res, next) =>{
    ...
    jwt.verify(token, process.env.forget_sign, async (err, User) => {
        ...
        if(err) {
         ...
         user.updateOne({email: User.userId}, {
                password: encryptedPassword
...
exports.ResetDetails = catchAsync(async (req, res, next) => {
    ...
    if(! (password && oldEmail)) {
        return next(new AppError("You have to enter your password and email to change user details", 403));
    }
